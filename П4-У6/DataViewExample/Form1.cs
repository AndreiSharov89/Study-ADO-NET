...
namespace DataViewExample
{
    public partial class Form1 : Form
    {
        ...
        {
            customersTableAdapter1.Fill(nortwndDataSet1.Customers);
            ordersTableAdapter1.Fill(nortwndDataSet1.Orders);

            customersDataView = new DataView(nortwndDataSet1.Customers);
            ordersDataView = new DataView(nortwndDataSet1.Orders);
        ...
        {
            string selectedCustomerID = (string)dataGridView1.SelectedCells[0].OwningRow.Cells["CustomerID"].Value;
            DataRowView selectedRow = customersDataView[customersDataView.Find(selectedCustomerID)];
            ordersDataView = selectedRow.CreateChildView(nortwndDataSet1.Relations
                                          ["FK_Orders_Customers"]);
